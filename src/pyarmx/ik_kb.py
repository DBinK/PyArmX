import time
import keyboard
import mujoco
import mujoco.viewer
import numpy as np
from scipy.spatial.transform import Rotation as R


class MujocoIK:
    """Mujoco 逆运动学求解器"""
    
    def __init__(self, model, data, site_name, arm_dof, pos_weight=1.0, rot_weight=0.3, max_iters=8, step_max=0.08, pos_tol=1e-4, rot_tol=2e-3):
        """初始化 IK 求解器并保存模型相关状态和超参数"""
        self.model = model
        self.data = data
        self.site_id = self.model.site(site_name).id
        self.arm_dof = arm_dof
        
        # 提取关节范围用于裁剪
        self.q_min = self.model.jnt_range[:self.arm_dof, 0].copy()
        self.q_max = self.model.jnt_range[:self.arm_dof, 1].copy()
        
        # 保存 IK 求解超参数
        self.pos_weight = pos_weight
        self.rot_weight = rot_weight
        self.max_iters = max_iters
        self.step_max = step_max
        self.pos_tol = pos_tol
        self.rot_tol = rot_tol

    @staticmethod
    def rotation_error(R_current, R_target):
        """计算旋转矩阵的旋转误差"""
        R_err = R_target @ R_current.T
        return 0.5 * np.array([
            R_err[2, 1] - R_err[1, 2],
            R_err[0, 2] - R_err[2, 0],
            R_err[1, 0] - R_err[0, 1],
        ])

    @staticmethod
    def clamp_norm(x, max_norm):
        """限制向量的长度（限制速度）"""
        n = np.linalg.norm(x)
        if n > max_norm and n > 1e-12:
            return x / n * max_norm
        return x

    @staticmethod
    def adaptive_damping_from_svd(J, lam_min=1e-4, lam_max=5e-2, sigma_ref=0.05):
        """计算 SVD 的最小奇异值，并返回 damping 参数和最小奇异值"""
        s = np.linalg.svd(J, compute_uv=False)
        sigma_min = s[-1] if len(s) > 0 else 0.0
        ratio = np.clip(sigma_min / sigma_ref, 0.0, 1.0)
        lam = lam_max * (1.0 - ratio) ** 2 + lam_min
        return lam, sigma_min

    def solve(self, q_init, target_pos, target_rot):
        """执行 IK 迭代，返回目标关节角度"""
        q = q_init.copy()
        
        for _ in range(self.max_iters):
            # 前向运动学更新状态
            self.data.qpos[:self.arm_dof] = q
            mujoco.mj_forward(self.model, self.data)
            
            # 获取当前末端位姿
            current_pos = self.data.site_xpos[self.site_id].copy()
            current_rot = self.data.site_xmat[self.site_id].reshape(3, 3).copy()

            # 计算误差
            pos_err = target_pos - current_pos
            rot_err = self.rotation_error(current_rot, target_rot)

            # 误差在容差范围内则提前停止
            if np.linalg.norm(pos_err) < self.pos_tol and np.linalg.norm(rot_err) < self.rot_tol:
                break

            # 组合误差与雅可比矩阵
            err = np.concatenate([self.pos_weight * pos_err, self.rot_weight * rot_err])
            jacp = np.zeros((3, self.model.nv))
            jacr = np.zeros((3, self.model.nv))
            mujoco.mj_jacSite(self.model, self.data, jacp, jacr, self.site_id)
            
            J = np.vstack([self.pos_weight * jacp[:, :self.arm_dof], self.rot_weight * jacr[:, :self.arm_dof]])

            # 自适应阻尼与求解
            lam, _ = self.adaptive_damping_from_svd(J)
            H = J.T @ J + lam * np.eye(self.arm_dof)
            g = J.T @ err

            try:
                dq = np.linalg.solve(H, g)
            except np.linalg.LinAlgError:
                dq = np.linalg.pinv(H) @ g

            # 限制单步步长并裁剪关节限位
            dq = self.clamp_norm(dq, self.step_max)
            q = np.clip(q + dq, self.q_min, self.q_max)
            
        return q


# =========================
# 🚀 仿真主循环
# =========================
def main():
    # 加载模型
    # model = mujoco.MjModel.from_xml_path("xml/gripper/scene.xml")
    model = mujoco.MjModel.from_xml_path("xml/mjcf/scene.xml")
    data = mujoco.MjData(model)
    
    ARM_DOF = 6
    dt = model.opt.timestep
    mujoco.mj_forward(model, data)

    # 实例化 IK 求解器
    ik_solver = MujocoIK(
        model=model, 
        data=data, 
        site_name="ee", 
        arm_dof=ARM_DOF, 
        pos_weight=1.0, 
        rot_weight=0.1115, 
        max_iters=8
    )

    # 获取初始状态变量
    target_id = model.body("target").id
    target_pos = model.body_pos[target_id].copy()
    lock_rot = data.site_xmat[ik_solver.site_id].reshape(3, 3).copy()
    q_sol = data.qpos[:ARM_DOF].copy()

    # 控制参数
    target_speed = 0.15
    rot_speed = 1.0  # rad/s
    last_print_time = 0.0

    # 启动查看器
    with mujoco.viewer.launch_passive(model, data) as viewer:
        while viewer.is_running():
            t0 = time.perf_counter()

            # --- 键盘位置控制 ---
            move_dir = np.zeros(3)
            speed_scale = 4.0 if keyboard.is_pressed("space") else 1.0

            if keyboard.is_pressed("up"): move_dir[1] += 1.0
            if keyboard.is_pressed("down"): move_dir[1] -= 1.0
            if keyboard.is_pressed("left"): move_dir[0] -= 1.0
            if keyboard.is_pressed("right"): move_dir[0] += 1.0
            if keyboard.is_pressed("alt"): move_dir[2] += 1.0
            if keyboard.is_pressed("ctrl"): move_dir[2] -= 1.0

            if np.linalg.norm(move_dir) > 1e-12:
                target_pos += (move_dir / np.linalg.norm(move_dir)) * target_speed * speed_scale * dt

            # --- 键盘姿态控制 (旋转轴) ---
            rot_vec = np.zeros(3)
            if keyboard.is_pressed("-"): rot_vec[0] -= 1.0
            if keyboard.is_pressed("="): rot_vec[0] += 1.0
            if keyboard.is_pressed("["): rot_vec[1] -= 1.0
            if keyboard.is_pressed("]"): rot_vec[1] += 1.0
            if keyboard.is_pressed(";"): rot_vec[2] -= 1.0
            if keyboard.is_pressed("'"): rot_vec[2] += 1.0

            if np.linalg.norm(rot_vec) > 1e-12:
                delta_ang = rot_vec * rot_speed * speed_scale * dt
                delta_rot_mat = R.from_rotvec(delta_ang).as_matrix()
                lock_rot = delta_rot_mat @ lock_rot

            # --- 重置位姿 ---
            if keyboard.is_pressed("r"):
                mujoco.mj_forward(model, data)
                lock_rot = data.site_xmat[ik_solver.site_id].reshape(3, 3).copy()
                target_pos = data.site_xpos[ik_solver.site_id].copy()

            # 更新可视化目标位置
            model.body_pos[target_id] = target_pos

            # --- 执行封装后的 IK 求解 ---
            q_sol = ik_solver.solve(q_sol, target_pos, lock_rot)

            # --- 驱动与仿真 ---
            data.ctrl[:ARM_DOF] = q_sol
            mujoco.mj_step(model, data)
            viewer.sync()

            # --- 打印误差 ---
            now = time.perf_counter()
            if now - last_print_time > 0.1:
                current_rot = data.site_xmat[ik_solver.site_id].reshape(3, 3)
                r_err_norm = np.linalg.norm(ik_solver.rotation_error(current_rot, lock_rot))
                p_err_norm = np.linalg.norm(target_pos - data.site_xpos[ik_solver.site_id])
                print(f"\rPos Err: {p_err_norm:.4f} | Rot Err: {r_err_norm:.4f}", end="")
                last_print_time = now

            # 保持实时频率
            elapsed = time.perf_counter() - t0
            time.sleep(max(0.0, dt - elapsed))


if __name__ == "__main__":
    main()